...
public class MpuserService {
	...
	/**
	 * 更新或者新增
	 * @param mpuser:  属性 id 为空时：  appid 和 openid 都不能为空； 属性id 不为空事， 随便啦
	 * @return
	 */
	public MpUser updateOrNew(MpUser mpuser){
		if(mpuser == null || (mpuser.getId() == null && (Strings.isNullOrEmpty(mpuser.getAppId()) || Strings.isNullOrEmpty(mpuser.getOpenId())))){
			return null;
		}
		synchronized (locker) {
