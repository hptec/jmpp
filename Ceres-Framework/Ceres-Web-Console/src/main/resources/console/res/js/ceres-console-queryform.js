consoleApp.directive('queryForm', function() {
	return {
		restrict : 'E',
		templateUrl : '/$$ceres_sys/console/base/res/cp?id=console/res/templates/console/queryform.html',
		compile : function(element, attrs) {
			var e = $(element);
			if (attrs.form == undefined || attrs.form == "") {
				return;
			}
			var id = Math.uuid(32);
			$(element).attr("id", id);
			var formDef = $c.queryForms[attrs.form];

			// 添加搜索条件
			if (formDef.terms.length > 0) {
				var contTerm = $("div[ceres-form-term]");
				for ($ln in formDef.terms) {
					var term = formDef.terms[$ln];
					switch (term.type) {
					case "ENUM":
						str = "<div class='col-md-2'>";
						str += "<select class='form-control' ng-model='$$ceresQueryForm.terms." + term.key + "'  ng-options='s.key as s.desc for  s in " + term.enumClass + "'>";
					...
					case "DATERANGE":
						...
						str += "</div>";
						str += "<input type='text' readonly ceres-queryform-term-type='DATERANGE' ng-model='$$ceresQueryForm.terms." + term.key + "' class='form-control pull-right' placeholder='"
								+ term.title + "'>";
						str += "</div>";
						str += "</div>";
						str += "</div>";
					...
					case "REMOTELIST":
						str = "<div class='col-md-2'>";
						str += "<select class='form-control'  ng-model='$$ceresQueryForm.terms." + term.key + "'  ng-options='s." + term.remoteList.keyName + " as s." + term.remoteList.descName
								+ " for  s in " + term.remoteList.listVarName + "'>";
						str += "<option value>" + term.title + "</option></select>";
						str += "</div>";
						contTerm.append(str);
						break;
