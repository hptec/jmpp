package cn.cerestech.framework.support.requirejs.html5mode;
...
public class AngularJsHtml5ModeSupport extends ResponseEntityExceptionHandler implements ComponentDispatcher {
		...
		if (cookies.exist(WebSupport.COOKIE_CERES_PLATFORM)) {
			...
			if (platform != null && indexPages.containsKey(platform)) {
				...
				for (Method m : methods) {
					if (m.isAnnotationPresent(RequireJsBootstrap.class)) {
						try {
	...
	public void recive(String beanName, Object bean) {
		if (bean instanceof WebSupport) {
			...
			for (Method m : mList) {
				if (m.isAnnotationPresent(RequireJsBootstrap.class)) {
					RequireJsBootstrap boot = m.getAnnotation(RequireJsBootstrap.class);
					if (indexPages.get(boot.platform()) != null) {
						throw new IllegalArgumentException("RequireJsBootstrap conflict: " + boot.platform());
					} else {
						indexPages.put(boot.platform(), bean);
					}
